...
{
    private void pDoPatternElementUI_position(int pattern, int index)
    {
        ...
        // Update find buttons.
        btn_posXRef.Enabled = commonVars.stitcher.getPatternElement(pattern, index)
            .getInt(PatternElement.properties_i.xPosRef, 0) > 0;
        btn_posYRef.Enabled = commonVars.stitcher.getPatternElement(pattern, index)
            .getInt(PatternElement.properties_i.yPosRef, 0) > 0;

    }

    private void pDoPatternElementUI_rotation(int pattern, int index)
    {
        ...
        int rp_val = rB_rotPivot_worldOrigin.Checked ? 2: 0;
        ...
        if (rRef >= 0)
        {
            ...
            rp_val = rB_rotPivot_ref.Checked ? 1 : rp_val;
        }
        commonVars.stitcher.getPatternElement(patternIndex: pattern, index).setInt(PatternElement.properties_i.refPivot, rp_val);
        ...
        btn_rotRef.Enabled = commonVars.stitcher.getPatternElement(pattern, index)
            .getInt(PatternElement.properties_i.rotationRef, 0) > 0;
    }

    private void pDoPatternElementUI_transform(int pattern, int index)
    {
    ...
    private void pDoPatternElementUI_array(int pattern, int index)
    {
        bool isArray = false;
        bool isRelativeArray = false;

        bool bounding = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.shapeIndex) == (int)CentralProperties.shapeNames.bounding;
        ...
        int arp_val = rB_arrayRotPivot_worldOrigin.Checked ? 2: 0;
        ...
        if (aRRef >= 0)
        {
            ...
            arp_val = rB_arrayRotPivot_ref.Checked ? 1: arp_val;
        }
        
        commonVars.stitcher.getPatternElement(patternIndex: pattern, index).setInt(PatternElement.properties_i.refArrayPivot, arp_val);
        ...
        // Update find buttons.
        btn_arrayRef.Enabled = isRelativeArray;
        btn_arrayRotRef.Enabled = aRRef >= 0;
    }
}