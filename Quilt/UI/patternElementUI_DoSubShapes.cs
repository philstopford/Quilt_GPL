...
public partial class MainForm
{
    ...
    {
        int ref_ = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.MinHTRef, 0);
        bool refActive = ref_ != 0;
        num_layer_minht.Enabled = !refActive;
        cb_ht_final.Enabled = refActive;
        if (!refActive)
        {
            cb_ht_final.Checked = false;
        }
        
        ref_ = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.HTIncRef, 0);
        refActive = ref_ != 0;
        num_layer_incHT.Enabled = !refActive;
        
        ref_ = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.HTStepsRef, 0);
        refActive = ref_ != 0;
        num_layer_stepsHT.Enabled = !refActive;
        
        ref_ = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.MinVTRef, 0);
        refActive = ref_ != 0;
        num_layer_minvt.Enabled = !refActive;
        cb_vt_final.Enabled = refActive;
        if (!refActive)
        {
            cb_vt_final.Checked = false;
        }
        
        ref_ = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.VTIncRef, 0);
        refActive = ref_ != 0;
        num_layer_incVT.Enabled = !refActive;
        
        ref_ = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.VTStepsRef, 0);
        refActive = ref_ != 0;
        num_layer_stepsVT.Enabled = !refActive;
        
        ref_ = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.tipRef, 0);
        refActive = ref_ != 0;
        comboBox_tipLocations.Enabled = !refActive;
        
        ref_ = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.tipRef, 1);
        refActive = ref_ != 0;
        comboBox_tipLocations2.Enabled = !refActive;
        
        ref_ = commonVars.stitcher.getPatternElement(patternIndex: pattern, index).getInt(PatternElement.properties_i.tipRef, 2);
        refActive = ref_ != 0;
        comboBox_tipLocations3.Enabled = !refActive;
    }
    ...
    private void pRefState_subShape2(int pattern, int index)
    {
        ...
        comboBox_s1_tip_ref.Enabled = true;
        comboBox_s1_tip_subShapeRef.Enabled = true;
    ...
    private void pRefState_subShape3(int pattern, int index)
    {
        ...
        comboBox_s2_tip_ref.Enabled = true;
        comboBox_s2_tip_subShapeRef.Enabled = true;
            ...
            else
            {
                switch (shapeString)
                {
                    case "bounding":
                        ...
                        break;
                    case "complex":
                        groupBox_properties.Content = new Expander()
                            {Content = groupBox_layout_table, Header = "Edges", Expanded = false};
                        break;
                }
