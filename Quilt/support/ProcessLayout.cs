...
public class ProcessLayout
{
    ...
    private Dictionary<string, string> structureLDNames;
    ...
    private bool pProcessLayout(ref bool abortLoad, double tolerance, bool vertical)
    {
        ...
        for (int i = 0; i < structureLDs.Count; i++)
        {
            ...
            polydata = GeoWrangler.clockwiseAndReorderXY(polydata);
            ...
            for (int p = 0; p < polydata.Count; p++)
            {
                ...
                try
                {
                    ...
                    for (int offsetFromReference = offset; offsetFromReference > 0; offsetFromReference--)
                    {
                        ...
                        if (pattElements[elementReferenceIndex - 1].getInt(PatternElement.properties_i.shapeIndex) != (int)ShapeSettings.typeShapes_mode1.rectangle)
                        {
                            continue;
                        }
                        ...
                        eX -= rX;
                        pattElements[elementReferenceIndex].setInt(PatternElement.properties_i.xPosSubShapeRefPos, (int)ShapeSettings.subShapeHorLocs.L);
                        if (eX >= rW)
                        {
                            eX -= rW;
                            pattElements[elementReferenceIndex].setInt(PatternElement.properties_i.xPosSubShapeRefPos, (int)ShapeSettings.subShapeHorLocs.R);
                        }

                        eY -= rY;
                        pattElements[elementReferenceIndex].setInt(PatternElement.properties_i.yPosSubShapeRefPos, (int)ShapeSettings.subShapeVerLocs.B);
                        if (eY >= rH)
                        {
                            eY -= rH;
                            pattElements[elementReferenceIndex].setInt(PatternElement.properties_i.yPosSubShapeRefPos, (int)ShapeSettings.subShapeVerLocs.T);
                        }
